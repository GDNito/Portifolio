...
const about = [
  ...
  "Médio integrado ao Técnico - Informatica para Internet",
...
const Courses = () => {
  const courseItems = courses.map((course, index) => (
    ...
    >
      <h2 className="text-sm md:text-xl">{course}</h2>
      <h2 className="text-xs md:text-base">{locations[index]}</h2>
      <h3 className="text-xs md:text-sm">{durations[index]}</h3>
      <h3 className="text-sm md:text-[1.5vw] lg:text-[1.2vw] xl:text-[1vw]">
        {about[index]}
      </h3>
    </div>
  ));
